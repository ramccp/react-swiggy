...
const RestaurantContainer = () => {
  ...
  useEffect(() => {
    fetch(API_URL)
      ...
      .then((obj) => {
        setResData(
          obj.data?.cards[1]?.card?.card?.gridElements?.infoWithStyle
            ?.restaurants
        );
        setFilteredData(
          obj.data?.cards[1]?.card?.card?.gridElements?.infoWithStyle
    ...
    <div>
      <div id="btns" className="w-10/12 mx-auto mt-4">
        <button
          ...
          className="filter-btn bg-green-300 py-2 px-6 rounded-md hover:cursor-pointer"
        >
          Filter
        </button>
        <button
          onClick={clearFilter}
          className="clear-btn bg-red-600 text-white py-2 px-6 mx-4 rounded-md hover:cursor-pointer hover:bg-red-500"
        >
          Clear
        </button>
        <input
          ...
          placeholder="Search"
          ref={searchRef}
          className="bg-slate-100 placeholder:text-black text-black outline-none py-2 px-6 mx-4 rounded-md"
        />
        <input
          type="button"
          value={"Search"}
          onClick={() => console.log(searchRef.current.value)}
          className="bg-amber-600 text-white py-2 px-6 mx-4 rounded-md hover:cursor-pointer"
        />
        <span>{status ? "🟢" : "🔴"}</span>
      </div>
